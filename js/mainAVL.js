class Main {
    constructor(canvas, databaseMisc) {
        ...
        this.randAVLValueCheckbox = document.getElementById(canvas.id+":random-AVL-value");
        ...
        /** The list of AVL values shown to the student and lecturer */
        this.AVLValueList = document.getElementById(canvas.id+":AVL-values");
        this.AVLTools = document.getElementById(canvas.id+":AVL-tools");
        ...
        this.qTypes = {TRAVERSAL: "traversal", AVL: "AVL", PROPERTIES: "properties"};
        //#endregion
        ...
        this.helpText = {
            ...
            AVL:        '<p>This question requires you to build a binary search tree using the values displayed in the "AVL Values" box. ' +
    ...
    initListeners() {
        ...
        this.randAVLValueCheckbox.addEventListener("change", this.randAVLValueChecked.bind(this, this.randAVLValueCheckbox)); // Bind Main class and actual checkbox element
    ...
    randNodeValueChecked(checkbox) {
        ...
        this.randAVLValueCheckbox.checked = false; // Uncheck random AVL if random input is checked
        this.randAVLValue = false;
    ...
    randAVLValueChecked(checkbox) {
        this.randAVLValue = checkbox.checked;
        this.randNodeValueCheckbox.checked = false; // Uncheck random input if random AVL is checked
        this.randNodeValue = false;

        if(this.randAVLValue) { // If the random value checkbox is checked, disable user specified node values
            this.nodeValueInput.disabled = true;
        }
        else {
            this.nodeValueInput.disabled = false;
    ...
    addRoot() {
        ...
        if(!this.databaseMisc.lecturer && this.databaseMisc.qtype === this.qTypes.AVL) {
            if(this.databaseMisc.AVLvalues.length === 0) return;
            
            newNodeValue = Number(this.nodeValueInput.value);
            this.nodeValueInput.value = this.attempt.AVLAttempt.AVL.values[this.attempt.AVLAttempt.AVL.getIndex("next")];
            this.attempt.AVLAttempt.AVL.undoButton.style.display = "inline-block";
        }
        else {
        ...
        if(this.databaseMisc.lecturer) {
            ...
            if(this.databaseMisc.qtype === this.qTypes.AVL) {
                this.attempt.AVLAttempt.AVL.stack.push(this.tree.root);
            }
    ...
    onBoardClick(event) {
        ...
        if(typeof this.tree.nodes[this.board.cellY][this.board.cellX] !== "undefined") { // There is a node at the selected cell
            ...
            if(this.databaseMisc.lecturer) {
                ...
                removeEventListener("keydown", this.onArrowClick.bind(this)); // Student cannot use arrow keys in a traversal question
                if(this.databaseMisc.qtype !== this.qTypes.AVL) { // Student can't edit node values or remove nodes in AVL question (can only undo)
                    this.removeNodeButton.style.display = "inline-block";
                    this.editNodeValueButton.style.display = "inline-block";
                }
                ...
                else if(this.databaseMisc.qtype === this.qTypes.AVL){ // Student can use arrow keys on AVL question
            ...
            if(!this.databaseMisc.lecturer && this.databaseMisc.qtype === this.qTypes.AVL) {
                newNodeValue = Number(this.nodeValueInput.value);
                this.nodeValueInput.value = this.attempt.AVLAttempt.AVL.values[this.attempt.AVLAttempt.AVL.getIndex("next")]; // The index will be attempt.AVL.values.length + 1 after adding the final AVL value (i.e. nodeValueInput will be an empty string)
            }
            else {
            ...
            if(this.databaseMisc.lecturer) {
                ...
                if(this.databaseMisc.qtype === this.qTypes.AVL) {
                    this.attempt.AVLAttempt.AVL.stack.push(this.tree.getNode(Number(newNodeValue)));
                }
        ...
        if(typeof this.tree.nodes[this.board.cellY][this.board.cellX] !== "undefined") { // There is a node in the hovered cell
            ...
            if(this.dragging) { 
                ...
                if (this.board.cellX == this.selectedNode.cellCoords.x || this.board.cellY <= this.selectedNode.cellCoords.y ||
                    ...
                    (!this.databaseMisc.lecturer && this.databaseMisc.qtype === this.qTypes.AVL && this.attempt.AVLAttempt.AVL.stack.length === this.attempt.AVLAttempt.AVL.values.length) || (this.databaseMisc.lecturer && !this.getNewNodeValue())) { // Invalid cell to place new child
    ...
    getNewNodeValue(root = false, edit = false) {
        ...
        if(!this.randNodeValue && !this.randAVLValue) { // Set the node's value to the user specified input
        ...
        else if(this.randAVLValue) { // Generate a random value for the node that enforces the AVL property of the tree
