class Main {
    constructor(canvas, databaseMisc) {
        ...
        console.log("DATA", this.databaseMisc);
        ...
        //tools for building avl tree
        //elemts declared in lecturer.html
        ...
        //add avl to qtypes dictionary
        this.qTypes = {TRAVERSAL: "traversal", BST: "bst", PROPERTIES: "properties",AVL:"avl"};
        //#endregion
        ...
        this.helpText = {
            ...
            avl:        '<p>Construct avl tree using the provided nodes</p>'
        };
    ...
    addRoot() {
        ...
        if(!this.databaseMisc.lecturer && this.databaseMisc.qtype === this.qTypes.AVL) {
            ...
            this.nodeValueInput.value = this.attempt.avlAttempt.bst.values[this.attempt.avlAttempt.avl.getIndex("next")];
        ...
        if(this.databaseMisc.lecturer) {
            ...
            else if(this.databaseMisc.qtype === this.qTypes.AVL) {
    ...
    onBoardClick(event) {
        ...
        if(typeof this.tree.nodes[this.board.cellY][this.board.cellX] !== "undefined") { // There is a node at the selected cell
            ...
            if(this.databaseMisc.lecturer) {
                ...
                else if(this.databaseMisc.qtype !== this.qTypes.AVL) { // Student can't edit node values or remove nodes in BST question (can only undo)
                    this.removeNodeButton.style.display = "inline-block";
                    this.editNodeValueButton.style.display = "inline-block";
                }
                ...
                else if(this.databaseMisc.qtype === this.qTypes.AVL){ // Student can use arrow keys on BST question
                    addEventListener("keydown", this.onArrowClick.bind(this));
                }
            ...
            else if(!this.databaseMisc.lecturer && this.databaseMisc.qtype === this.qTypes.AVL) {
                newNodeValue = Number(this.nodeValueInput.value);
                this.nodeValueInput.value = this.attempt.avlAttempt.avl.values[this.attempt.avlAttempt.avl.getIndex("next")]; // The index will be attempt.bst.values.length + 1 after adding the final BST value (i.e. nodeValueInput will be an empty string)
            }
            else {
