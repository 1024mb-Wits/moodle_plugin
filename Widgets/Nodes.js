...
var startX, startY;
var addNodes, deleteNodes, connectNodes, moveNodes, DeleteSubT = false; // a boolean variable that determines whether the user wants to move the nodes or not
let n = 0; //Node number
...
function installMouseHandler() {
    ...
    function doMouseDown(evt) {
        ...
        if (Node_Selected.GetNodeNum() != -1 && Node_Selected.GetIs_Selected() && (Selected_Nodes[0] == -1 || Selected_Nodes[1] == -1)) {
            openPopup();
...
function DrawNewHeight(Node, Str) {

    graphics.save();
    graphics.font = "12px Arial";
    graphics.fillStyle = "black";
    graphics.fillText("Height:" + Str, Node.GetX() - 20, Node.GetY() - 38);
    graphics.restore();
...
function DeleteSubTree(evt) {
    nodenums = new Array();

    for (let i = 0; i < subtree.length; i++) {

        deleteNode(subtree[i].GetNodeNum(), subtree[i].GetArrIndex());
...
function addNode(posx, posy, num) {
    ...
    var New_Node = new Node(posx, posy, num, n);
...
function FxIndDel(Del_At) {
    for (let i = Del_At; i < Added_Nodes.length; i++) { //starting from where we deleted till the end of the array
        Added_Nodes[i].SetArrIndex(i);//pushing back the index by 1 since a deletion occured before it
    }
    return Added_Nodes;
}
...
function paintComponent(whichSelection) { // gets the operation from the user
    ...
    switch (whichSelection) {
        ...
        case "-1": // user has not chosen any option
            addNodes = false;
            moveNodes = false;
            deleteNodes = false;
            connectNodes = false;
        ...
        case "3": // user chooses to delete subtree
            addNodes = false;
            moveNodes = false;
            deleteNodes = false;
            connectNodes = false;
...
function TreetoNodes(arrTree) {
    for (let i = 0; i < arrTree.length; i++) {
        addNode(-1, -1, arrTree[i]);
        //  console.log("added: ",Added_Nodes[i]);
    }
}
...
function DrawTree() {
    let Off_val = 70;
    let LC_XOffset = -Off_val;
    let LC_YOffset = Off_val;
    let RC_XOffset = Off_val;
    let RC_YOffset = Off_val;
    //console.log(Added_Nodes[0].GetNodeNum());

    var root = Added_Nodes[1].GetNodeNum();

    Added_Nodes[1].setRoot(true);
    Added_Nodes[1].SetX(360);
    Added_Nodes[1].SetY(67);

    let parent = Added_Nodes[1];
    let new_edge;



    for (let i = 2; i < Added_Nodes.length; i++) {
        curr = Added_Nodes[i];
        // let n=new Node(parent.GetX()+LC_XOffset,parent.GetY()+LC_YOffset,curr);
        console.log(i);

        if (curr.GetNodeNum() < parent.GetNodeNum()) {
            if (parent.IsRoot()) {
                LC_XOffset = LC_XOffset - 80;
                Added_Nodes[i].SetX(parent.GetX() + LC_XOffset);
                Added_Nodes[i].SetY(parent.GetY() + LC_YOffset);
                LC_XOffset = LC_XOffset + 80;
            }
            else {
                Added_Nodes[i].SetX(parent.GetX() + LC_XOffset);
            ...
            Added_Nodes[parent.GetArrIndex()].SetLeftChild(Added_Nodes[i]);
            new_edge = new Edge(parent.GetNodeNum(), Added_Nodes[i].GetNodeNum());
            Added_Edges.push(new_edge);
            parent = Added_Nodes[i];
        }
        else {
            let largest = -1;
            let largest_ind = -1;
            console.log("checking largest");

            for (let j = i - 1; j >= 0; j--) {
                if (Added_Nodes[j].GetNodeNum() > largest) {
                    if (Added_Nodes[j].GetNodeNum() < Added_Nodes[i].GetNodeNum()) {
                        largest = Added_Nodes[j].GetNodeNum();
                        largest_ind = j;
                    }
                }
            }
            if (Added_Nodes[largest_ind].IsRoot()) {
                RC_XOffset = RC_XOffset + 80;
                Added_Nodes[i].SetX(Added_Nodes[largest_ind].GetX() + RC_XOffset);
                Added_Nodes[i].SetY(Added_Nodes[largest_ind].GetY() + RC_YOffset);
                RC_XOffset = RC_XOffset - 80;
            }
            else {
                Added_Nodes[i].SetX(Added_Nodes[largest_ind].GetX() + RC_XOffset);
            ...
            Added_Nodes[largest_ind].SetRightChild(Added_Nodes[i]);
            parent = Added_Nodes[i];
            console.log("largest", Added_Nodes[largest_ind].GetNodeNum());
            new_edge = new Edge(Added_Nodes[largest_ind].GetNodeNum(), Added_Nodes[i].GetNodeNum());
            Added_Edges.push(new_edge);
...
function ClearArrays() {
    ...
    for (let i = 1; i < Added_Nodes.length; i++) {
        //we need to delete this edge
        Added_Nodes.splice(i, 1);
        i--;
    }
}

function GetPreOrderTraversalString(ArrTrav) {
    TreeStr = '';
    for (let i = 0; i < ArrTrav.length - 1; i++) {
        TreeStr = TreeStr + ArrTrav[i] + ",";
    }

    TreeStr = TreeStr + ArrTrav[ArrTrav.length - 1];


    return TreeStr;
}

function Generate_AVL_TREE(num,proceed) {
    ClearArrays();
    n = 0;
    graphics.clearRect(0, 0, canvas.width, canvas.height);
    if (proceed) {
        //generate tree
        var ArrTrav = CreateTree(num);
        TreetoNodes(ArrTrav);
        DrawTree();
        return GetPreOrderTraversalString(ArrTrav);
    }
}
function Generate_AVL_Quest(ArrTrav) {
    ClearArrays();
    n = 0;
    graphics.clearRect(0, 0, canvas.width, canvas.height);
    //generate tree
...
function CheckHeights(){
    let Heights_Added=false;
    for (let i = 1; i < Added_Nodes.length; i++) {
        if (Added_Nodes[i].GetHeight() !== -1) {
            Heights_Added=true;
            break;
    }
...
function DrawAllHeights() {
    if (CheckHeights()){
        for (let i = 1; i < Added_Nodes.length; i++) {
            if (Added_Nodes[i].GetHeight() !== -1 && Added_Nodes[i].GetHeight()!='')  {

                DrawNewHeight(Added_Nodes[i], Added_Nodes[i].GetHeight());
            }
            else {
                DrawNewHeight(Added_Nodes[i], "__");
            }
        }
...
function AddNodeHeight() {
    let H = GetHeightValue();
    Node_Selected.SetHeight(H);
    console.log(Node_Selected.GetHeight());
    graphics.clearRect(0, 0, canvas.width, canvas.height);
