...
/**
 * Question type class for the avl question type.
 *
 * @package    qtype
 * @subpackage avl
...
require_once($CFG->dirroot . '/question/type/avl/question.php');


/**
 * The avl question type.
 *
 * @copyright 2022 adam gordon

 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_avl extends question_type {

      /* ties additional table fields to the database */
    public function extra_question_fields() {
        return array('question_avl', 'somefieldname','anotherfieldname');
    }
    ...
    public function save_question_options($question) {
        global $DB;
        $options = $DB->get_record('qtype_question_avl', array('questionid' => $question->id));
        if (!$options) {
        ...
        $DB->update_record('qtype_question_avl', $options);
 ...
 **/
   ...
   public function initialise_question_answers(question_definition $question, $questiondata,$forceplaintextanswers = true){ 
    ...
    public function import_from_xml($data, $question, qformat_xml $format, $extra = null) {
        if (!isset($data['@']['type']) || $data['@']['type'] != 'question_avl') {
            return false;
        }
    ...
    public function export_to_xml($question, qformat_xml $format, $extra = null) {
        ...
        $gapfillinfo = $pluginmanager->get_plugin_info('question_avl');
